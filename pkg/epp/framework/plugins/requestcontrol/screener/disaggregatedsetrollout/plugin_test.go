...
func TestScreenerWithoutRevisionGatingIsReady(t *testing.T) {
	config := validConfig()
	config.RevisionGating = &RevisionGating{Mode: GatingModeDisabled}
	if err := newTestScreener(config).CheckReady(); err != nil {
		t.Fatalf("CheckReady() with disabled revision gating: %v", err)
	}
}
